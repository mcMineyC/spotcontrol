package ap

import (
	"crypto"
	"crypto/rsa"
	"crypto/sha1"
	"math/big"
)

// serverSigPubKey is the well-known RSA public key used by Spotify access
// points to sign the server's Diffie-Hellman public key during key exchange.
var serverSigPubKey *rsa.PublicKey

func init() {
	serverSigPubKey = &rsa.PublicKey{
		N: new(big.Int).SetBytes([]byte{
			,
		}),
		E: 65537,
	}
}

// verifySignature checks that sig is a valid PKCS#1 v1.5 SHA-1 signature over
// data, using the well-known Spotify server signature public key. It returns
// true if the signature is valid.
func verifySignature(data []byte, sig []byte) bool {
	hashed := sha1.Sum(data)
	err := rsa.VerifyPKCS1v15(serverSigPubKey, crypto.SHA1, hashed[:], sig)
	return err == nil
}
